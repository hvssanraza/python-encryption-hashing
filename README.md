...
This repository demonstrates **encryption, decryption, and hashing
techniques in Python** using Google Colab.\
It includes practical examples of the **CIA Triad** (Confidentiality,
Integrity, Availability) and the **Avalanche Effect** in hashing.
...
| `encrypted_file.txt` | Encrypted version of `file.txt` (ciphertext) |
| `decrypted_file.txt` | Decrypted version of the encrypted file (should match original) |
...
2.  Upload a sample `file.txt` to your Drive.\
...
🔄 **Comparison:** All values are different → proves **Avalanche
Effect** ⚡
...
-   **Confidentiality** → Encryption ensures only key holders can access
    the data.\
-   **Integrity** → Hashing ensures detection of even the smallest
    modification.\
-   **Availability** → Files accessible anytime via Google Drive +
    Colab.
...
> A **tiny change** in input (e.g., adding one word to a file) causes a
> **completely different hash output**.\
> This property ensures high sensitivity of hashing functions to data
> integrity.
